...
class LocationBooking {
    ...
    setupBookingButton() {
        // Connect to the calendar overlay instead of the old modal system
        const triggerButtons = document.querySelectorAll('.booking-trigger-btn');

        triggerButtons.forEach(button => {
            button.addEventListener('click', (e) => {
                e.preventDefault();
                // Use the global calendar overlay function
                if (window.showCalendarOverlay) {
                    window.showCalendarOverlay();
                } else {
                    console.warn('Calendar overlay not available');
                }
            });
        });
    }
...
// Global initialization function for location page
window.initializeLocationPage = function() {
    // Only initialize if we're on the location page and content is loaded
    if (!document.getElementById('location')) return;
    ...
    // Define calendar functions locally
    function getDayBoxStyle(date) {
        const dayOfWeek = date.getDay();
    ...
    function updateBookingPanel() {
        ...
        if (timeScheduleContainer) timeScheduleContainer.innerHTML = ''; // Clear old inputs
        ...
        sortedDates.forEach(dateStr => {
            ...
            dateInputGroup.innerHTML = `
                ...
                <div class="time-validation-error" id="error-${dateStr}"></div>
            `;
            if (timeScheduleContainer) timeScheduleContainer.appendChild(dateInputGroup);
        });

        // Add validation listeners
        if (timeScheduleContainer) {
            timeScheduleContainer.querySelectorAll('.time-input').forEach(input => {
                input.addEventListener('input', validateTimePair);
            });
        }
    ...
    function validateTimePair(e) {
        ...
        if (!startInput || !endInput || !startInput.value || !endInput.value) return; // Not enough info yet

        const startTime = parseTime(startInput.value);
        const endTime = parseTime(endInput.value);

        if (startTime >= endTime) {
            if (errorEl) errorEl.textContent = 'L\'heure de fin doit être après l\'heure de début.';
            return false;
        }
        ...
        if (durationMinutes < 30) {
            if (errorEl) errorEl.textContent = 'La durée minimale est de 30 minutes.';
            return false;
        }

        if (durationMinutes > 240) { // 4 hours
            if (errorEl) errorEl.textContent = 'La durée maximale est de 4 heures.';
            return false;
        }
    ...
    function clearAllSelections() {
        selectedDates.clear();
        if (bookingPanel) bookingPanel.style.display = 'none';
        if (timeScheduleContainer) timeScheduleContainer.innerHTML = '';
    ...
    // Re-render calendar if it's empty
    ...
    // Re-setup phone number formatting
    const phoneInput = document.getElementById('bookingPhone');
    if(phoneInput) {
        phoneInput.addEventListener('input', (e) => {
            let numbers = e.target.value.replace(/\D/g, '');
    ...
    // Initialize responsive calendar if it exists
    const responsiveCalendarContainer = document.getElementById('responsive-calendar-container');
    if (responsiveCalendarContainer && responsiveCalendarContainer.innerHTML.trim() === '') {
        // Initialize ResponsiveMonthlyCalendar if available
        if (window.ResponsiveMonthlyCalendar) {
            new ResponsiveMonthlyCalendar(responsiveCalendarContainer);
        }
    }
};