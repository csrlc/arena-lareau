...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Render full year calendar (September 2025 to August 2026)
    function renderFullYearCalendar() {
        const calendarGrid = document.querySelector('.events-full-calendar-grid');
        if (!calendarGrid) return;

        calendarGrid.innerHTML = '';

        // Define months from September 2025 to August 2026
        const months = [
        ...
        months.forEach(({ month, year }) => {
            ...
            monthContainer.className = 'events-month-container-full';
            ...
            monthContainer.innerHTML = `
                <h3 class="events-month-header-full">${monthName.charAt(0).toUpperCase() + monthName.slice(1)}</h3>
                <div class="events-weekdays-mini-full">
                    <div>Dim</div><div>Lun</div><div>Mar</div><div>Mer</div><div>Jeu</div><div>Ven</div><div>Sam</div>
                </div>
                <div class="events-days-grid-mini-full" data-month="${month}" data-year="${year}"></div>
            `;
    ...
    function renderMonthDaysFull(month, year) {
        ...
        const daysGrid = document.querySelector(`.events-days-grid-mini-full[data-month="${month}"][data-year="${year}"]`);
        if (!daysGrid) return;
        ...
        // Empty cells before month starts
        for (let i = 0; i < startingDayOfWeek; i++) {
            daysHTML += '<div class="events-day-mini-full blank"></div>';
        }
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const classes = ['events-day-mini-full'];
            let style = '';
            let title = formatDate(date);
    ...
    // Render full year calendar instead of just two months
...
// Global initialization function for events page
window.initializeEventsPage = function() {
    // Re-run the calendar and event rendering when page becomes active
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    // Re-render full year calendar
    const calendarGrid = document.querySelector('.events-full-calendar-grid');
    if (calendarGrid && calendarGrid.innerHTML.trim() === '') {
        // Define months from September 2025 to August 2026
        const months = [
            { month: 8, year: 2025 },   // September 2025
            { month: 9, year: 2025 },   // October 2025
